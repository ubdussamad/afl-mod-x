...
unsigned int *guiding_graph_hashes = (void *)0 ;
unsigned int number_of_branch_hits = 0;
unsigned long num_of_guiding_graph_hashes = 0;
...
#ifdef ENABLE_SYSCALL_TARGETTING
  double branch_weight; /* Describes closeness to a branch  */
                        /* with syscall in it.              */
#endif                  /* ENABLE_SYSCALL_TARGETTING */
...
#ifdef ENABLE_SYSCALL_TARGETTING
static void add_to_queue(u8 *fname, u32 len, u8 passed_det, float branch_weight)
#else
...
#endif
{
  ...
  #ifdef ENABLE_SYSCALL_TARGETTING
  // Every new queue will have 0.0 as default weight.
  q->branch_weight = branch_weight;
  #endif
...
static void read_testcases(void)
{
  ...
  for (i = 0; i < nl_cnt; i++)
  {
    ...
    ck_free(dfn);
#ifdef ENABLE_SYSCALL_TARGETTING
    add_to_queue(fn, st.st_size, passed_det, 0.1 );
#else
  ...
  if (fault == crash_mode)
  {
    ...
    /* Keep only if there are new bits in the map, add to queue for
      ...
      for (u16 gg_idx=0; gg_idx < num_of_guiding_graph_hashes; gg_idx++) {
        ...
        // printf("No guiding graph hashes found.\n");
        prefrence_weight = 1.0;
      }
#endif
...
#endif /* ^!SIMPLE_FILES */
#ifdef ENABLE_SYSCALL_TARGETTING
    add_to_queue(fn, len, 0, prefrence_weight);
#else
...
#ifndef ENABLE_SYSCALL_TARGETTING
  sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "american fuzzy lop", use_banner);
#else
  if (num_of_guiding_graph_hashes == 0) {
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "american fuzzy lop [SYSCALL MOD, UN-GUIDED]", use_banner);
  } else {
    sprintf(tmp + banner_pad, "%s " cLCY VERSION cLGN " (%s)", crash_mode ? cPIN "peruvian were-rabbit" : cYEL "american fuzzy lop [SYSCALL MOD, GUIDED]", use_banner);
  }
#endif /* ^!ENABLE_SYSCALL_TARGETTING */
...
#ifdef ENABLE_SYSCALL_TARGETTING
  while ((opt = getopt(argc, argv, "+i:a:o:f:m:b:t:T:dnCB:S:M:x:QV")) > 0)
#else
...
#ifdef ENABLE_SYSCALL_TARGETTING
    case 'a': /* Hash guide file. */
        printf("File %s will be used as a hash file.\n", optarg);
        ...
        // Allocate memory for hashes and weights
        guiding_graph_hashes = (unsigned int *)malloc(num_of_guiding_graph_hashes * sizeof(unsigned int) +1);
        guiding_graph_hash_weights = (float *)malloc(num_of_guiding_graph_hashes * sizeof(float) +1 );
        rewind(fp);
        int rs;
        for (int i = 0; i < num_of_guiding_graph_hashes; i++) {
            rs = fscanf(fp, "%d,%f", &guiding_graph_hashes[i], &guiding_graph_hash_weights[i]);
            if (rs == EOF) {
        ...
        // // Print the hashes
        // for (int i = 0; i < num_of_guiding_graph_hashes; i++) {
        //     printf("Hash: %d, Weight: %f\n", guiding_graph_hashes[i], guiding_graph_hash_weights[i]);
        // }
        break;
#endif
